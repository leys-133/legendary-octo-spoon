...
const STUDENT = {
    ...
    updateStats() {
        ...
        document.getElementById('quran-progress').textContent = quranPercentage + '%';

        // Hadith count
        const hadithCount = (user.hadithProgress || []).length;
        document.getElementById('hadith-count').textContent = hadithCount;

        // Lessons watched
        const lessonsWatched = (user.lessonsWatched || []).length;
        document.getElementById('lessons-watched').textContent = lessonsWatched;

        // Reports sent
        const reportsSent = (user.reports || []).length;
        document.getElementById('reports-sent').textContent = reportsSent;
    },
